...
public class EmpleadosAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> implements Filterable {

    public List<EmpleadosModel> info;
    public List<EmpleadosModel> aux;
    ...
    public EmpleadosAdapter(List<EmpleadosModel> info, Context context, OnItemClickListener mItemClickListener) {
        this.info = info;
    ...
    public void setInfo(List<EmpleadosModel> info) {
        this.info = info;
        this.aux = new ArrayList<>(info);
    }
    ...
    public void onBindViewHolder(@NonNull RecyclerView.ViewHolder holder, int position) {
        if (holder instanceof ItemAdapterViewHolder) {
            final EmpleadosModel cls = info.get(position);
            ItemAdapterViewHolder holderItem = (ItemAdapterViewHolder) holder;
            holderItem.lblCVCodigo.setText(String.valueOf(cls.getCODIGO()));
            holderItem.lblCVDescripcion.setText(cls.getNOMBRE() +" " + cls.getApellido());
    ...
    private Filter getBuscar = new Filter() {
        ...
        protected FilterResults performFiltering(CharSequence charSequence) {
            List<EmpleadosModel> filteredList = new ArrayList<>();
            if (charSequence == null || charSequence.length() == 0) {
                filteredList.addAll(aux);
            } else {
                String filterPattern = charSequence.toString().toLowerCase().trim();
                for (EmpleadosModel item : aux) {
